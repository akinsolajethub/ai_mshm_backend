...
class CheckinSessionService:
    ...
    def complete_session(session: CheckinSession) -> DailyCheckinSummary:
        ...
        IMPORTANT: This method must NEVER raise an exception.
        All external calls (Redis, Celery, WebSocket) are wrapped in try/except.
        """
        # 1. Mark session complete
        session.mark_complete()
        logger.info(
            "Completed %s session for %s on %s",
            session.period,
            session.user.email,
            session.checkin_date,
        )

        # 2. Assemble daily summary
        summary = DailySummaryService.assemble_or_update(session.user, session.checkin_date)

        # 3. Update streak — wrap in try/except
        try:
            StreakService.update(session.user, session.checkin_date)
        except Exception as e:
            logger.warning("Streak update skipped for %s: %s", session.user.email, e)

        # 4. Send notification — wrap in try/except
        try:
            ...
            logger.warning("Notification send skipped for %s: %s", session.user.email, e)

        # 5. Queue prediction if ready — wrap in try/except
        if summary.is_ready_for_prediction and not summary.prediction_run:
            try:
                ...
                logger.warning("Prediction task skipped for %s: %s", session.user.email, e)
    ...
    def submit_hrv(
        ...
        session.save(
            update_fields=["hrv_sdnn_ms", "hrv_rmssd_ms", "hrv_captured_at", "hrv_skipped"]
        )

        # Update summary HRV fields too
        try:
            summary = DailyCheckinSummary.objects.get(
                user=session.user, summary_date=session.checkin_date
            )
            if not skipped:
...
class DailySummaryService:
    ...
    def assemble_or_update(user: User, summary_date: date) -> DailyCheckinSummary:
        ...
        recent_mfg = (
            HirsutismMFGCheckin.objects.filter(
                user=user,
        ...
        logger.info(
            ...
            user.email,
            summary_date,
            summary.morning_complete,
            summary.evening_complete,
        )
        return summary
