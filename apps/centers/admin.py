...
class HealthCareCenterAdmin(admin.ModelAdmin):
    ...
    fieldsets = (
        ...
        (
            "Escalation Routing",
            {
                "fields": ("escalates_to_state_hospital",),
                "description": (
                    "IMPORTANT: Set this to the State General Hospital in the same state as this PHC. "
                    "This is the routing link — Moderate/Severe patients from this PHC "
                    "will be escalated to this hospital. If not set, the system falls back "
                    "to any active State General Hospital in the same state."
                ),
            },
        ),
        (
    ...
    def get_form(self, request, obj=None, **kwargs):
        """
        Filter escalates_to_state_hospital to show only active State Hospitals.
        Adds help text showing the PHC's current state for easy selection.
        """
        form = super().get_form(request, obj, **kwargs)
        if "escalates_to_state_hospital" in form.base_fields:
            from .models import StateHospital

            qs = StateHospital.objects.filter(status=StateHospital.CenterStatus.ACTIVE).order_by(
                "state", "name"
            )
            form.base_fields["escalates_to_state_hospital"].queryset = qs
            state_hint = obj.state if (obj and obj.state) else "(set state field first)"
            form.base_fields["escalates_to_state_hospital"].help_text = (
                f"Only active State General Hospitals shown. This PHC is in state: {state_hint}. "
                f"Select a State Hospital in the same state for correct routing."
            )
        return form
